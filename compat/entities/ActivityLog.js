...
export const ActivityLog = {
  ...
  let d = _read()
    ...
    if(limit)d=d.slice(0,limit);return d
  },
  async get(id){return _read().find(r=>r.id===id)||null},
  ...
  d[i]={...d[i],...patch,updated_date:new Date().toISOString()};_write(d);return d[i]},
  async remove(id){
    _write(_read().filter(r=>r.id!==id));return{ok:true}
  }
};
